...
my_doc <- my_doc %>% 
  body_add_par("I. English Literature", style = "Section Header")
...
main <- subset(stss, `Q3`== 'English Literature')

sec <- subset(stss, `Q4` == 'English Literature' )
...
my_doc <- my_doc %>% 
  body_add_par("Chart I1 Academic sessions (XX respondents)", style = "Normal") %>% 
...
my_doc <- my_doc %>% 
  body_add_par("Chart I2 The academic staff (XX respondents)", style = "Normal") %>% 
  body_add_img(src = src, width = 6,  height = 3)
...
my_doc <- my_doc %>% 
  body_add_par("Chart I3 The group project sessions (XX respondents)", style = "Normal") %>% 
  body_add_img(src = src, width = 6,  height = 3)
...
my_doc <- my_doc %>% 
  body_add_par("Chart I4 Overall, how would you rate the Sutton Trust Summer school at UoE? (XX respondents)", style = "Normal") %>%
...
my_doc <- my_doc %>% 
  body_add_par("Chart I5 Overall, are you more or less likely to consider applying to ANY university now? (XX respondents)", style = "Normal") %>%
  body_add_img(src = src, width = 6,  height = 3, style = "Normal")
...
text(barplot(Q32_1_a$Freq, col = c("red","green","blue","purple","yellow"),ylim=c(0,120),
             las=1,
             border=NA,
...
my_doc <- my_doc %>% 
  body_add_par("Chart I6 Overall, are you more or less likely to consider applying to UoE now? (XX respondents)", style = "Normal") %>%
  body_add_img(src = src, width = 6, height = 3, style = "Normal")

print(my_doc, target = "EL18.docx")
