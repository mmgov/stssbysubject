...
my_doc <- my_doc %>% 
  body_add_par("C. Ecological & Earth Sciences", style = "Section Header")
...
main <- subset(stss, `Q3`== 'Ecological & Earth Sciences')

sec <- subset(stss, `Q4` == 'Ecological & Earth Sciences' )
...
my_doc <- my_doc %>% 
  body_add_par("Chart B1 Academic sessions (XX respondents)", style = "Normal") %>% 
...
my_doc <- my_doc %>% 
  body_add_par("Chart B2 The academic staff (XX respondents)", style = "Normal") %>% 
  body_add_img(src = src, width = 6,  height = 3)
...
my_doc <- my_doc %>% 
  body_add_par("Chart B3 The group project sessions (XX respondents)", style = "Normal") %>% 
  body_add_img(src = src, width = 6,  height = 3)
...
my_doc <- my_doc %>% 
  body_add_par("Chart B4 Overall, how would you rate the Sutton Trust Summer school at UoE? (XX respondents)", style = "Normal") %>%
...
my_doc <- my_doc %>% 
  body_add_par("Chart B5 Overall, are you more or less likely to consider applying to ANY university now? (XX respondents)", style = "Normal") %>%
  body_add_img(src = src, width = 6,  height = 3, style = "Normal")
...
my_doc <- my_doc %>% 
  body_add_par("Chart B6 Overall, are you more or less likely to consider applying to UoE now? (XX respondents)", style = "Normal") %>%
  body_add_img(src = src, width = 6, height = 3, style = "Normal")
