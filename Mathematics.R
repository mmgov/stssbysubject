...
my_doc <- my_doc %>% 
  body_add_par("F. Mathematics", style = "Section Header")
...
main <- subset(stss, `Q3`== 'Mathematics')

sec <- subset(stss, `Q4` == 'Mathematics' )
...
my_doc <- my_doc %>% 
  body_add_par("Chart F1 Academic sessions (XX respondents)", style = "Normal") %>% 
...
my_doc <- my_doc %>% 
  body_add_par("Chart F2 The academic staff (XX respondents)", style = "Normal") %>% 
  body_add_img(src = src, width = 6,  height = 3)
...
my_doc <- my_doc %>% 
  body_add_par("Chart F3 The group project sessions (XX respondents)", style = "Normal") %>% 
  body_add_img(src = src, width = 6,  height = 3)
...
text(barplot(Q31$Freq, col = c("red","green","blue","purple","yellow"),ylim=c(0,120),
             las=1,
             border=NA,
             names.arg=Q31$Var1),
...
my_doc <- my_doc %>% 
  body_add_par("Chart F4 Overall, how would you rate the Sutton Trust Summer school at UoE? (XX respondents)", style = "Normal") %>%
...
text(barplot(Q32_2_a$Freq, col = c("red","green","blue","purple","yellow"),ylim=c(0,120),
             las=1,
             border=NA,
...
my_doc <- my_doc %>% 
  body_add_par("Chart F5 Overall, are you more or less likely to consider applying to ANY university now? (XX respondents)", style = "Normal") %>%
  body_add_img(src = src, width = 6,  height = 3, style = "Normal")
...
my_doc <- my_doc %>% 
  body_add_par("Chart F6 Overall, are you more or less likely to consider applying to UoE now? (XX respondents)", style = "Normal") %>%
  body_add_img(src = src, width = 6, height = 3, style = "Normal")



print(my_doc, target = "M18.docx")
